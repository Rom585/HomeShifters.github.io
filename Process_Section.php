<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['approve_moving'])) {
        ...
        $sql = mysqli_query($conn, "INSERT INTO approved_moving (moving_to, moving_from, fullname, email, phone_number, quote_date, moving_date, moving_package) 
        ...
        $id = intval($_POST['id']);
        $sql = mysqli_query($conn, "DELETE FROM processed_moving WHERE id = '$id'");
        if ($sql) {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['approve_storage'])) {
        $fullname = $_POST['fullname'];
        $email = $_POST['email'];
        $phone_number = $_POST['phone_number'];
        $quote_date = date('Y-m-d', strtotime($_POST['quote_date']));
        ...
        // Insert moving data into new database
        $sql = mysqli_query($conn, "INSERT INTO approved_storage (fullname, email, phone_number, quote_date, storage_date, storage_location, storage_package) 
        VALUES ('$fullname', '$email', '$phone_number', '$quote_date', '$storage_date', '$storage_location', '$storage_package')");

        // Delete the processed row from the original table
        $id = intval($_POST['id']);
        $sql = mysqli_query($conn, "DELETE FROM processed_storage WHERE id = '$id'");
        if ($sql) {
            echo "<script>window.alert('Moving data processed and deleted successfully!'); window.location.href='Process_Section.php';</script>";
        } else {
            echo "<script>window.alert('Error deleting moving data: ); window.location.href='AdminUi.php.php';</script>";
        }
        $process->close();
...
<div class="sidebar">
    ...
    <ul class="nav">
        ...
        <li><a href="Process_Section.php" class="active">Processing</a></li>
        ...
        <div class="search-box">
            ...
            <div class="data-grid-container">
                ...
                <table class="data-grid">
                    ...
                    <tbody>
                        <?php
                        $sql = mysqli_query($conn, "SELECT * FROM processed_moving");
                    ...
                    <tr>
                        ...
                        <td>
                            <form action="" method="post">
                                ...
                                <button type="submit" class="process-btn" name="approve_moving">Done</button>
                            </form>
                        </td>
                    </tr>
                    <?php } ?>
                ...
                <table class="data-grid">
                    ...
                    <tbody>
                        <?php
                        $sql = mysqli_query($conn, "SELECT * FROM processed_storage");
                        while ($row = mysqli_fetch_array($sql)) {  
                        ?>
                    <tr>
                        <td><?php echo $row[0]; ?></td>
                        ...
                        <td>
                            <form action="" method="post">
                                ...
                                <input type="hidden" name="storage_date" value="<?php echo $row[4]; ?>">
                                <input type="hidden" name="storage_location" value="<?php echo $row[5]; ?>">
                                <input type="hidden" name="storage_package" value="<?php echo $row[6]; ?>">
                                <button type="submit" class="process-btn" name="approve_storage">Done</button>
                            </form>
                        </td>
                    </tr>
                    <?php } ?>
